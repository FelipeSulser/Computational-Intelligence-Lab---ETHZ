...
def extract_data(filename, num_images, starting_id, context_factor):
    ...
    for i in range(starting_id, num_images+starting_id):
        if 'test_set' in filename:
            image_filename = filename + 'test_'+str(i)+'/test_'+str(i)+'.png'
        else:
...
STARTING_ID = 1
TRAINING_SIZE = 200
data_dir = (os.path.dirname(os.path.realpath(__file__)))+'/test_set_images/'
...
im_save(save_dir+'test_mean_img.png', mean_img)
