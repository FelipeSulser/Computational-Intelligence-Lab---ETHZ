...
def img_crop_context(im, w, h,context_factor, stdev, normalize = True):
    ...
    for i in range(cf,imgheight-cf,h):
        for j in range(cf,imgwidth-cf,w):
            ...
            if normalize:
                im_patch = im_patch  - np.mean(im_patch)
                im_patch = im_patch / stdev
...
def extract_data(filename, num_images, starting_id, context_factor,stdev):
    ...
    img_patches = [img_crop_context(imgs[i], IMG_PATCH_SIZE, IMG_PATCH_SIZE,context_factor, stdev, normalize=True) for i in range(num_images)]
    data = [img_patches[i][j] for i in range(len(img_patches)) for j in range(len(img_patches[i]))]
    data = np.asarray(data)
    return data

def getStd(filename, num_images, starting_id):
    """Extract the images into a 4D tensor [image index, y, x, channels].
    Values are rescaled from [0, 255] down to [-0.5, 0.5].
    """
    imgs = []
...
def extract_labels(filename, num_images, starting_id, context_factor):
    ...
    context_factor = 0
    gt_patches = [img_crop_context(gt_imgs[i], IMG_PATCH_SIZE, IMG_PATCH_SIZE,context_factor,STDEV, normalize=False) for i in range(num_images)]
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    datasetStdev = getStd(train_data_filename, TRAINING_SIZE, STARTING_ID)
    STDEV = datasetStdev

    train_data = extract_data(train_data_filename, TRAINING_SIZE, STARTING_ID, CONTEXT_ADDITIVE_FACTOR,STDEV)
    ...
    def get_prediction(img):
        cropped = img_crop_context(img, IMG_PATCH_SIZE, IMG_PATCH_SIZE,CONTEXT_ADDITIVE_FACTOR,STDEV, normalize=True)
